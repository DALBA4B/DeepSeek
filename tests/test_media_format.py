...
def test_build_media_hint_explicit_gif_request_ignores_probability():
    ...
    hint = _build_media_hint("скинь гифку", media_probability=0.0, gif_probability=0.0)
    assert "GIPHY" in hint


def test_build_media_hint_explicit_sticker_request_ignores_probability():
    hint = _build_media_hint("дай стикер", media_probability=0.0, gif_probability=0.0)
    assert "STICKER" in hint


def test_build_media_hint_no_request_and_zero_probabilities_is_empty():
    hint = _build_media_hint("привет как сам", media_probability=0.0, gif_probability=0.0)
    assert hint == ""
...
def test_build_media_hint_no_request_but_gif_probability_one_always_gif():
    ...
    hint = _build_media_hint("привет как сам", media_probability=0.0, gif_probability=1.0)
    assert "GIPHY" in hint
...
def test_build_media_hint_no_request_but_media_probability_one_gives_sticker_or_react():
    ...
    hint = _build_media_hint("привет как сам", media_probability=1.0, gif_probability=0.0)
    assert hint != ""
