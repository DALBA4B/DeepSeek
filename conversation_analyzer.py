...
needs_memory — нужны ли факты об ЭТОМ чате и его людях. Длина ответа тут не \
при чём: короткий вопрос может требовать памяти, а развёрнутый — нет.
  true:
    названо ЛЮБОЕ имя или прозвище человека — считай, что это участник чата, \
а не публичная личность. В базе есть годы переписки, там про него точно есть.
    упомянуто прошлое, привычки, вещи, отношения кого-то из своих
    триггеры: "помнишь", "вчера", "тот раз", "ты знал что", "кто из нас"
    вопрос о том, что уже обсуждалось в чате
    человек говорит что-то о себе или своей жизни — "я купил", "я вчера", \
"я устал", "у меня". Даже если ответа тут на одно слово: этот факт нужно \
сверить с тем, что уже известно про него.
  false:
    общие знания — объясни физику, напиши код, придумай историю, переведи
    (в базе про это ничего нет, запрос только потратит время)
    пустая реакция без содержания: "ахаха", "ну да", смайл, стикер
