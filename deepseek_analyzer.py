...
DEEPSEEK_ANALYSIS_PROMPT = """Ты извлекаешь ОБЪЕКТИВНЫЕ ФАКТЫ О ЛИЧНОСТИ из сообщений пользователя.

Проанализируй сообщения пользователя {username} и выпиши только ПОСТОЯННЫЕ ХАРАКТЕРИСТИКИ И ИНТЕРЕСЫ.
...
{{
    ...
    "interests": {{
        ...
        "movies": ["фильм 1"],
        "sports": ["спорт 1"],
        "music": ["жанр/исполнитель 1"],
        "work": ["профессия/должность"],
        "other": ["интерес 1"]
    }}
}}
...
2. "Сейчас болею" → исключить (временное состояние)
3. "Устал" → исключить (эмоция в момент)
4. "Грущу" → исключить (настроение, не факт)
5. "Завтра пойду на учёбу" → исключить (план, не интерес)

ЗОЛОТОЕ ПРАВИЛО:
- НЕ ДОБАВЛЯЙ СВОИ ВЫВОДЫ И ИНТЕРПРЕТАЦИИ
- ПИШИ ТОЛЬКО ТО, ЧТО СКАЗАНО, КАК СКАЗАНО
