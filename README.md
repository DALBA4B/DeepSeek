...
формата, без heuristика «реакция или текст» (`Responder.send_silent_reaction`)
— если реакцию поставить не удалось, бот просто остаётся молчать, а не
подменяет её текстом (не встревать в разговор — весь смысл фичи).
