...
- Python 3.9+ (`zoneinfo` для часовых поясов)
...
> Фаза B (LightRAG, см. `plans/PHASE_B_PLAN.md`).
...
- Если явной просьбы нет — `brain._build_media_hint()` разрешает модели
  один случайный медиа-ответ на этот ход, с двумя НЕЗАВИСИМЫМИ шансами:
  `gif_response_probability` (по умолчанию 1.5% — гифка реже, это
  сетевой запрос к Giphy, может тормозить/падать) и
  `media_response_probability` (по умолчанию 5% — стикер или реакция).
- В остальных случаях (по умолчанию) в промпт добавляется явный ЗАПРЕТ
  (`_NO_MEDIA_HINT`: «только обычный текст, без GIPHY:/REACT:/STICKER:»),
  а не просто отсутствие разрешающей строки — модель надёжнее слушается
  явного запрета рядом с вопросом, чем общего правила из начала промпта.
...
- **Код-гарантия на случай если модель всё равно ослушалась**: если медиа
  было запрещено, а ответ всё же пришёл как `GIPHY:`/`REACT:`/`STICKER:`,
  `Brain.generate_response()` не отдаёт это пользователю — просит модель
  переответить обычным текстом ещё раз; если и повторная попытка вернула
  медиа-формат, бот в этот раз просто молчит (`None`), а не шлёт то, что
  не должно было прийти.
- Стикер подбирается по эмоции ответа (`StickerManager` в `responder.py`
  сопоставляет запрошенную эмоцию с `sticker.emoji` из загруженных паков),
  а не берётся случайно из всех стикеров.
- **Несколько стикерпаков**: `load_sticker_set()` можно вызвать несколько
  раз (список паков — `STICKER_PACKS` в `main.py`) — стикеры накапливаются
  из всех паков вместе, а не заменяют друг друга. Найти точное короткое
  имя пака (оно не всегда совпадает с тем, что видно в приложении) можно
  через `tools/sticker_finder.py` — временную утилиту, слушающую любой
  присланный стикер и печатающую его `set_name`.
- **Не повторяет недавние стикеры**: `StickerManager` помнит последние 10
  отправленных `file_id` (`RECENT_HISTORY_SIZE`) и исключает их при выборе
  — если совпадающих по эмоции стикеров достаточно, повтор не прилетит
  подряд; если пул слишком маленький — лучше повтор, чем совсем ничего.

Реакции (`REACT:<эмодзи>`) ограничены реальным списком, который принимает
Telegram (`prompts.ALLOWED_REACTION_EMOJIS`, ~70 эмодзи) — вне его
`set_reaction` падает с `REACTION_INVALID`. Модель явно видит этот список
в промпте и должна выбирать только из него; на всякий случай
`responder.py` дополнительно проверяет эмодзи ПЕРЕД вызовом Telegram API
(`_ALLOWED_REACTIONS`) — если модель всё же ошиблась, бот не тратит
API-вызов впустую, а сразу уходит в текстовый fallback (для обычного REACT)
или просто не реагирует (для "тихой" реакции, у неё нет текстового fallback
по дизайну). Обычные `😂`/`💀` в этот список НЕ входят — ближайший валидный
аналог смеха у Telegram это `🤣`.

Минимальная защита от спама одной и той же реакцией: `RecentReactionTracker`
в `responder.py` не даёт одному эмодзи сработать **5-й раз подряд** (после
4 одинаковых подряд следующий такой же превращается в обычный текстовый
ответ) — не мешает случайным повторам через сообщение-два, только длинным
сериям. Явные просьбы пользователя ("поставь реакцию") эту защиту обходят
— если просят конкретно, повтор не наш вопрос.

Отдельно: когда классификатор решил `grade=0` (бот бы полностью промолчал —
например кто-то шутит в чате, но не с ботом), с небольшим шансом
`silent_reaction_probability` (по умолчанию 3%) бот вместо полного молчания
просто ставит эмодзи-реакцию на сообщение, без единого вызова DeepSeek на
генерацию текста. Это `SILENT_REACT:` — отдельный путь от обычного REACT:
формата, без heuristика «реакция или текст». Здесь эмодзи выбирается не
моделью, а из фиксированного пула по ситуации (`brain._silent_reaction_pool`
— шутка/подкол/дефолт, 3-5 вариантов каждый), и уже сам `Responder`
(`send_silent_reaction_from_pool`) выбирает конкретный эмодзи из пула с
учётом того же `RecentReactionTracker` — если реакцию поставить не удалось,
бот просто остаётся молчать, а не подменяет её текстом (не встревать в
разговор — весь смысл фичи).
...
| Команда | Действие |
|---------|----------|
| `/on` / `/off` | Полностью выключить/включить бота из чата (молча; выключенный бот не пишет в память и не тратит токены) |
| `/help` | Список доступных команд (работает и у выключенного бота) |
| `/mood` (ответом на сообщение) | Текущий уровень «обиды» бота на этого человека (`GrudgeTracker`) |
| `/mem <вопрос>` | Прямой запрос в LightRAG: возвращает сгенерированный LightRAG ответ, а не факты для промпта |

Скрытые/служебные (не показываются в `/help`, но работают): `/ragstats`, `/ragnow`.
...
### LightRAG: таймаут запроса и формат ответа

- `LIGHTRAG_QUERY_TIMEOUT` (по умолчанию 35с) — сколько ждать ответа от
  `/query`. Раньше было 8с, из-за чего составные запросы (несколько имён
  сразу) не успевали и молча считались «фактов не найдено», хотя факты в
  базе были. Если база вырастет (например, до года переписки), может
  понадобиться поднять ещё — смотри реальные тайминги в `RAG-DEBUG` логах
  (см. ниже).
- `only_need_context=True` в `rag_client.retrieve()` — LightRAG отдаёт
  сырой контекст (сущности/связи/чанки) без своей собственной генерации
  ответа. Раньше стояло `False`, и LightRAG на каждый запрос гонял ещё
  одну LLM, чтобы сформулировать полный ответ, который потом всё равно
  переписывался DeepSeek бота в своём голосе — двойная генерация впустую,
  и именно она в основном ела время сверх самого поиска.
...
MEDIA_RESPONSE_PROBABILITY=0.05  # Шанс неожиданного стикера/реакции без явной просьбы
GIF_RESPONSE_PROBABILITY=0.015   # То же самое, но для гифки (реже — сетевой запрос)
```

Длина ответа отдельно не настраивается: она привязана к грейду
классификатора в `Brain._tokens_for_grade()` (grade 1 → короткая реакция,
grade 2 → обычный ответ, grade 3 → развёрнутый) вместе с температурой —
это осознанное решение, чтобы код управлял форматом, а не модель.
...
### Подробная отладка RAG-памяти (`RAG-DEBUG`)

`brain.py`, `conversation_analyzer.py`, `rag_client.py` умеют логировать
весь путь сообщения по шагам — классификация → запрос в LightRAG → сборка
промпта → генерация ответа, включая сырые payload'ы, найденные факты,
токены и тайминги каждого шага. По умолчанию это **выключено** (иначе на
каждое сообщение в лог льётся полный системный промпт + факты + ответ —
слишком много для повседневной работы).

Включить: `RAG_DEBUG_LOGGING=true` в `.env`, перезапустить бота. Строки
помечены `RAG-DEBUG` и пишутся отдельным логгером `ragdebug` — можно
смело держать выключенным и включать точечно только когда реально нужно
разобрать, почему память нашла/не нашла факты или почему ответ странный.
